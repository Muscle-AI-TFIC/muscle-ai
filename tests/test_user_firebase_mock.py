...
def test_user_firebase_mock_empty():
    ...
    with patch("firebase_admin.credentials.Certificate"), \
         ...
         patch("firebase_admin.auth.list_users", return_value=mock_user_firebase):
...
def test_user_firebase_mock_with_users():
    ...
    with patch("firebase_admin.credentials.Certificate"), \
         patch("firebase_admin.initialize_app"), \
         patch("firebase_admin.auth.list_users", return_value=mock_user_list):

        users = get_user_firebase()

        # verifing if the datas match
        assert users[0] == {
            "id": "10",
